...
Everything else is Ushida's words.\\[0.5em]
...
\begin{editorialnote}
The section title \jp{穂足} (\textit{hosoku}) literally means
``grain-spike form''---\jp{穂} (\textit{ho}, ear of rice) plus
\jp{足} (\textit{ashi}, form/trace/print). In modern Japanese trading
terminology, \jp{足} is the standard suffix for visual price
representations: \jp{ローソク足} (\textit{r\=osoku-ashi},
``candlestick''), \jp{日足} (daily bar), \jp{月足} (monthly bar),
\jp{かぎ足} (hook chart), and so on. The suffix denotes the visual
trace that price movement leaves behind.

The term \jp{穂足} appears to be the earliest documented use of a
\jp{足}-suffix term for visual price pattern recognition in Japanese,
predating the Meiji-era \jp{ローソク足} (candlestick) terminology by
roughly 140 years. The visual parallel is striking: an ear of rice
(\jp{稲穂}) has a thick central body (the grain cluster) with thin
projections (the awns)---the same morphology as a candlestick chart
element (thick body, thin wicks). Rice traders in the 1750s naturally
described price patterns using rice imagery; when chart-makers
formalized visual representations in the 1890s--1900s, the metaphor
shifted to a household object (the candle), but the underlying concept
of \jp{足}---the visual footprint of price---remained the same.

No scholar appears to have previously connected \jp{穂足} to the later
\jp{ローソク足} tradition. Steve Nison, who introduced candlestick
charting to the West in 1991, later acknowledged (1994, p.\,14) that
candlestick charts likely originated in the Meiji period, not the Edo
period. The anchor chart (\jp{いかり足}) is documented from the
Ky\=oh\=o era (c.\,1716). The \jp{穂足} of this text
(c.\,1755) sits in the same generation but uses an agricultural
metaphor drawn directly from the rice trade itself.
\end{editorialnote}
...
\caption[Position-scaling tree diagrams]{Position-scaling tree diagrams from the original manuscript.
...
\caption[The turnover diagram]{The turnover diagram
(\jp{転愛商内之圖}\protect\footnotemark, \textit{Diagram of
...
\label{fig:tree2}
\end{figure}
\footnotetext{The diagram's title cartouche reads \jp{転愛商内之圖}.
The body text uses \jp{転換の法} (\textit{tenkan no h\=o}, ``turnover
method'') for the same concept. The character \jp{愛} in the title may
be a variant or cursive form of \jp{換}; both readings are preserved
...
in trading positions. This passage appears in the manuscript within the
``Following Gold'' (\jp{随金}) section (page 27); it is placed here
under ``Discipline'' because it states the virtue's core principle.} of twenty tawara and trades with five will survive
