...
\begin{editorialnote}
\jp{穂足} (\textit{hosoku}): literally ``grain-spike form''---\jp{穂}
(ear of rice) plus \jp{足} (form/trace). In modern Japanese trading,
\jp{足} is the standard suffix for price representations:
\jp{ローソク足} (candlestick), \jp{日足} (daily bar), \jp{かぎ足}
(hook chart), etc. Whether \jp{穂足} is an early instance of this
convention is an open question---Edo-period traders used bar charts
(\jp{棒足}) and anchor charts (\jp{いかり足}) rather than
candlesticks, which are a Meiji-era development. But the shared
\jp{足} suffix and the agricultural metaphor (an ear of rice is
morphologically similar to a candlestick element) are suggestive. No
scholar appears to have previously noted this term.
\end{editorialnote}
