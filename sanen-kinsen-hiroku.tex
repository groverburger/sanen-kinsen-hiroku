...
transcription. The internal consistency of the text---where concepts
introduced in the preface are operationalized in the trading rules,
quantified in the calculation tables, and synthesized in the grand
diagram---provides confidence that the overall structure and teachings
are accurately captured. Individual prose passages, particularly in
dense cursive sections, may reflect close paraphrase rather than
word-for-word fidelity. Uncertain readings are noted throughout.\\[1em]
...
manuscript's structure requires explanation. Chapter lengths vary because
...
Everything else is Ushida's words.\\[1em]

\noindent\textbf{Open Source}\\[0.3em]
\noindent The complete methodology behind this translation---including
scans of the original manuscript, character-by-character transcription
with confidence levels, working translation files, and the \LaTeX{}
source---is available as an open-source repository at
\url{https://github.com/groverburger/sanen-kinsen-hiroku}. Every
editorial decision is traceable and auditable.
\end{small}
\newpage
...
This is the family tradition of Ushida Gonzabur\=o.\tn{Two units recur
throughout the text that follows. \jp{両} (\textit{ry\=o}): the standard
gold coin of the Edo period, used here as the unit of trading capital.
\jp{俵} (\textit{tawara}): a bale of rice, the standard unit of measure
for rice trading and price quotation. Gold and rice---ry\=o and
tawara---are the two axes of every calculation in this text.}
...
\caption{The turnover diagram (\jp{転愛商内之圖}, \textit{Diagram of
Revolving Trading}). A 1,000-ry\=o buying position (\jp{千両買}, bottom)
scales through four stages (\jp{壱}--\jp{四}) to a 2,000-ry\=o selling
position (\jp{二千両売}, top). \textit{Image: Kyoto University Main
Library.}}
\label{fig:tree2}
\end{figure}
...
\jp{甲午}.\tn{As an example of how these map to Western dates:
\jp{甲子} (\textit{kinoe-ne}) corresponds to 1744, 1804, 1864 in the
Edo period; \jp{甲申} (\textit{kinoe-saru}) to 1764, 1824. Each
designation recurs every sixty years.} In these years, rice markets
experience extreme volatility; prices may surge two or three times
...
traditional Confucian system names five constant virtues (\jp{五常}), the
fifth being Trustworthiness (\jp{信}, \textit{shin}). Whether the fifth
was omitted deliberately, lost in transmission, or treated elsewhere in
the tradition is unknown from this manuscript. --- The manuscript heading
for this section reads \jp{仁 — 壱首・壱百五拾目} (\textit{Jin --- Isshu,
hyaku goj\=u me}), which appears to mean ``Benevolence---First article;
the 150th item.'' This reference to 150 articles (or items) is
unexplained in the manuscript and may indicate that this text is drawn
from, or claims to summarize, a larger collection of trading rules. The
relationship to the ``Fifteen Articles'' earlier in the text is
unclear---the numbers 15 and 150 may or may not be connected.}
...
level, using Osaka market rates (\jp{大坂数位}):\tn{The percentage
figures (\jp{一割}, \jp{二割}, \jp{三割}---10\%, 20\%, 30\%) in these
tables appear to refer to the \textit{market's price movement} from the
middle star level, not to returns on capital. That is: ``at 10\%'' means
``when the price has moved 10\% from the middle star, enter the first
position.'' The large tawara figures (\jp{三万余世俵}, 30,000+;
\jp{八万余}, 80,000+) likely represent cumulative trading volume across
the full market cycle rather than a single position's size. The
manuscript's accounting method---how capital, position size, and price
movement compound across stages---is not fully transparent, and some
figures in the cursive script are difficult to read with certainty.
What follows presents the manuscript's figures as legible, without
attempting to force them into a coherent modern accounting framework.}
...
Regardless of starting capital, every example converges toward
approximately 800 ry\=o in effective exposure. The key principle holds at
every level: the larger the capital base, the more critical the staged
method becomes.
...
\begin{center}
\jp{半板売米諸勘定之仕法}\\
\textit{Hanita Urimai Shokanjo no Shih\=o}
\end{center}
...
\item CiNii Books. \jp{校正三猿金泉秘録} (\textit{K\=osei Sanen Kinsen
  Hiroku}). Bibliographic record. National Institute of Informatics,
  Japan. \url{https://ci.nii.ac.jp/}
...
Zach Booth is a software engineer at NASA Ames Research Center, where he
works on mission planning for the VIPER Lunar Rover. He studies and
works with AI systems---including Claude (Anthropic)---in his daily
engineering workflow.

Self-taught in Japanese since the age of eighteen and conversational in
the language, Booth identified this manuscript in the Kyoto University
Rare Materials Digital Archive and recognized that no accurate English
translation existed. He directed the translation process using AI
assistance, applying the same methodology of systematic verification and
traceable decision-making that characterizes his engineering work.
